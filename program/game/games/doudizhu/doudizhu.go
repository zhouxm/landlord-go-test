...
func (dou *Doudizhu) AddPlayer(currPlayer game.IPlayer) error {
	...
	if len(dou.Players) == dou.playerNum {
		...
		logger.Info("有新玩家加入桌子" + strconv.Itoa(dou.id))
		i := len(dou.Players) - 1
		dou.Unlock()
		game.BindPlayerGame(currPlayer, dou)
		currPlayer.SetIndex(i)
...
func (dou *Doudizhu) RemovePlayer(player game.IPlayer) error {
	...
	logger.Info("游戏" + strconv.Itoa(dou.id) + "移除玩家" + strconv.Itoa(player.GetPlayerUser().Id))
	return nil
}
...
func (dou *Doudizhu) PlayerReady(p game.IPlayer) {
	...
	//用户都准备好了，则发牌
	if userAllReady {
		logger.Info("桌子" + strconv.Itoa(dou.id) + "的玩家都准备好了")
		dou.IsPlaying = true
		dou.Unlock()
		dou.dealCards()
	} else {
...
func (dou *Doudizhu) callLoardEnd() {
	...
	dou.CalledLoardNum = 0
	dou.Unlock()
	logger.Info("叫地主结束" + strconv.Itoa(dou.lordIndex) + "成为地主")
		...
		if dou.lastCards == nil || //第一次出牌
			...
			if p.IsOutOfCards() {
				...
				if currIndex == dou.lordIndex {
					dou.gameOver()
					logger.Info("地主第一个出完牌,游戏结束")
					return
				} else {
					dou.gameOver()
					logger.Info("一个农民出完牌,游戏结束")
					return
				}
			}
			//下一个玩家出牌
